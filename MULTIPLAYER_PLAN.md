...
Status: Phases 0 and 1 are complete on the `multiplayer-plan` branch. The Phase 2 transport, lobby, event journal, snapshot recovery, authenticated reconnect, complete gameplay-content manifest, and explicit first-contact fingerprint verification are implemented. Authority convergence is complete: live host and guest exploration inputs share the host command processor, replicas do not rerun rule-bearing pickup, attack, script, or timed-queue work, and periodic corrections cover actors, both progressing character builds, critters, doors, items, world time, game globals, indexed map/script variables, and timed queues. Installed-data two-process scenarios now cover guest pathfinding and movement through completion, asynchronous scripted-door use and its final state, the deferred pickup callback and inventory mutation, loot initiation, and host-authoritative party XP. Each applies a full authoritative checkpoint and compares every section digest; the engine probe also proves the door script and random combat attack run only in the host process. Phase 2.5 deterministic exploration coverage is complete. Combat wire primitives exist, but live multiplayer attacks are intentionally blocked until authoritative phase and turn ownership are implemented.
