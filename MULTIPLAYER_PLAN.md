...
- [ ] Add world-map travel while preserving one authoritative world clock and encounter state. The first prerequisite is in place: snapshot version 10 carries the persistent world-map grid, known town entrances, visited-city and special-encounter flags, position, town, and section, with a dedicated digest section and strict decoding. A two-process host-only mutation fixture proves the guest repairs world-map and encounter state through the checkpoint and replay. The legacy world-map UI remains blocked in multiplayer until the following parts are complete:
  1. [ ] Require every connected player to agree to enter the world map; one host-side travel controller owns input, clock advancement, queue processing, and encounter rolls.
  2. [ ] Publish a host-selected destination or encounter and player-keyed placements after map load, preserving remote actors and inventories; a canceled trip must restore the existing map and phase safely.
  3. [ ] Test town-to-town travel and an encounter in two installed-data processes, including state-digest convergence, guest-side zero rule execution, and authenticated replay.
