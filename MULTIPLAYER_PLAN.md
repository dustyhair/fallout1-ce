...
- [ ] Add world-map travel while preserving one authoritative world clock and encounter state. Snapshot version 12 carries the persistent world-map grid, known town entrances, visited-city and special-encounter flags, position, town, section, and recoverable travel-planning stage, original proposer, current controller, route target, and active travel counters, with a dedicated digest section and strict decoding. A two-process host-only mutation fixture proves the guest repairs world-map and encounter state through the checkpoint and replay. The world-map consent command stages a 90-second proposal in Exploration, requires the other player's approval before entering Transition, keeps the proposer as controller initially, permits withdrawal/rejection, and cancels pending consent on timeout, disconnect, combat, or map/phase change. Gameplay wire version 18 carries a bounded world-map target/clear intent; only the current controller can change it and the guest applies the host event without running travel rules. The legacy world-map UI remains blocked in multiplayer until travel execution is safe. The following parts remain:
  A headless host-only travel stepper is now extracted from the legacy loop: it follows terrain speed and walkmask, advances the clock through bounded queue boundaries, reveals map cells, heals and rolls encounters at daily intervals, and stops before encounter map loads or countdown movies. Snapshot version 12 carries and validates the active line, terrain/visual counters, day progress, and time rate against the approved route; recovery restores them without letting the guest execute travel rules. The installed-data modal smoke covers a host mid-route serialize/restore, guest execution guard, remote-player daily healing, a deterministic encounter, an interrupting due queue, and a pending countdown world event. The two-process world-map fixture makes the host move across city terrain and verifies that the guest repairs the resulting position through checkpoint/replay. It is not yet driven by route input or map-entry handling; full two-process encounter and queue-interruption convergence remain before live travel is enabled.
