...
- [ ] Add world-map travel while preserving one authoritative world clock and encounter state. The first prerequisite is in place: snapshot version 10 carries the persistent world-map grid, known town entrances, visited-city and special-encounter flags, position, town, and section, with a dedicated digest section and strict decoding. A two-process host-only mutation fixture proves the guest repairs world-map and encounter state through the checkpoint and replay. The legacy world-map UI remains blocked in multiplayer, and the generic shared-modal command cannot open it without consent. The following parts remain:
  1. [ ] Require every connected player to agree to enter the world map. The first player's request is the travel proposal; all other connected players must explicitly accept that proposal before the phase changes. A withdrawal, rejection, disconnect, map/phase change, or 90-second timeout cancels it without advancing the clock. Acceptance cannot transfer control to the accepting player.
  2. [ ] Give route and destination input to the proposing player, whether host or guest. The proposer may change waypoints/destination or cancel while travelling; the other player sees the proposed route and current travel state but cannot steer it. Send only bounded, validated travel intentions to the host. The host alone applies movement, world-clock advancement, due queues, healing, encounter rolls, and map-entry rules; the guest must never run those rules locally. This separates the input controller from game-state authority.
