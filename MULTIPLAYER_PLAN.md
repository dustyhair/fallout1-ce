...
Status: Phases 0 and 1 are complete on the `multiplayer-plan` branch. The Phase 2 transport, lobby, event journal, snapshot recovery, authenticated reconnect, complete gameplay-content manifest, and explicit first-contact fingerprint verification are implemented. Authority convergence is complete: live host and guest exploration inputs share the host command processor, replicas do not rerun rule-bearing pickup, skill, attack, script, or timed-queue work, and periodic corrections cover actors, both progressing character builds, critters, doors, non-door scenery, items and containers, world time, game globals, indexed map/script variables, and timed queues. Installed-data two-process scenarios now cover guest pathfinding and movement through completion, asynchronous scripted-door use and its final state, the deferred pickup callback and inventory mutation, loot initiation, a split cap gift, targeted door and scenery skills, concrete container/scenery state, and host-authoritative party XP. Each applies a full authoritative checkpoint and compares every section digest; the engine probe also proves the door script and random combat attack run only in the host process. Phase 2.5 deterministic exploration coverage is complete, and Phase 3A is in progress. Combat wire primitives exist, but live multiplayer attacks are intentionally blocked until authoritative phase and turn ownership are implemented.
...
5. [x] Add deterministic two-process exploration scenarios and structured completion assertions. Installed-data fixtures cover guest movement, scripted-door use, deferred pickup completion, loot initiation, a split cap gift, targeted skills, non-door scenery state, and container state, with full section-digest convergence and authenticated event replay.
...
Remaining Phase 3A work is to drive a small non-combat quest end to end and cover any map-object creation or destruction that quest exposes. Initial map scenery and items now have canonical identities and concrete recovery state; dynamically created objects still need an authoritative lifecycle before they can participate outside the existing inventory-transfer paths.
