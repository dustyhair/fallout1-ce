...
- [ ] Validate typed-stair behavior with a real installed or purpose-built fixture: independent same-map travel, all-player readiness rejection and shared cross-map travel, actor/inventory rebinding, full checkpoint convergence, and replay. The partial installed-map inspection found ladders but no typed stair object before the diagnostic scan stopped; it is not evidence that none exist elsewhere. Until this validation passes, typed-stair support is provisional.
...
- [ ] Extend agreed rest to the Pip-Boy's until-morning/noon/evening/midnight and until-healed choices, including interruption and healing-rate edge cases. Those options remain unavailable in co-op until host-authoritative behavior and tests exist.
- Add world-map travel while preserving one authoritative world clock and encounter state.
- [ ] Require readiness from every connected participant for exits, rest, and world-map transitions—both players in the MVP—with an explicit host timeout policy where appropriate. Ordinary exits require proximity and fixed-duration rest requires matching consent with a 90-second expiry; world-map policy remains. Same-map elevators remain independently usable, with proximity-based shared rides.
- Verify entity rebinding, every remote-player inventory, queued events, and state hashes across every transition boundary. The elevator and ordinary-exit installed-data scenarios currently cover this contract.
- [ ] Put transition readiness and destination placement behind roster-shaped
  ...
  fields. Ordinary exits now use the shared bounded player-keyed placement
  type; rest and world-map code must reuse that roster model rather than add
  another independent pair-shaped readiness or placement payload.
