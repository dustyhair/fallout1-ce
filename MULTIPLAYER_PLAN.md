...
Status: Phases 0 and 1 are complete on the `multiplayer-plan` branch. The Phase 2 transport, lobby, event journal, snapshot recovery, authenticated reconnect, complete gameplay-content manifest, and explicit first-contact fingerprint verification are implemented. Authority convergence is complete: live host and guest exploration inputs share the host command processor, replicas do not rerun rule-bearing pickup, attack, script, or timed-queue work, and periodic corrections cover actors, critters, doors, items, world time, game globals, indexed map/script variables, and timed queues. The installed-data two-process scenario executes a real guest facing command, applies a full authoritative checkpoint, compares every section digest, and proves a scripted door procedure and random combat attack run only in the host process. Phase 2.5 deterministic exploration coverage is now the active milestone; movement, door, pickup, and loot scenario fixtures remain. Combat wire primitives exist, but live multiplayer attacks are intentionally blocked until authoritative phase and turn ownership are implemented.
...
5. [ ] Add deterministic two-process exploration scenarios and structured completion assertions. The installed-data baseline now covers a real guest facing command plus full section-digest convergence; movement, door, pickup, and loot fixtures remain.
