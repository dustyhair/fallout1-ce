...
- [ ] Add world-map travel while preserving one authoritative world clock and encounter state. Snapshot version 10 carries the persistent world-map grid, known town entrances, visited-city and special-encounter flags, position, town, and section, with a dedicated digest section and strict decoding. A two-process host-only mutation fixture proves the guest repairs world-map and encounter state through the checkpoint and replay. The world-map consent command stages a 90-second proposal in Exploration, requires the other player's approval before entering Transition, keeps the proposer as controller, permits withdrawal/rejection, and cancels pending consent on timeout, disconnect, combat, or map/phase change. Gameplay wire version 18 also carries a bounded world-map target/clear intent; only the approved proposer can change it and the guest applies the host event without running travel rules. The legacy world-map UI remains blocked in multiplayer until travel execution is safe. The following parts remain:
  ...
  2. [ ] Connect the proposer-only target intent to world-map input, route changes, and peer presentation. The proposer may change waypoints/destination or cancel while travelling; the other player sees the proposed route and current travel state but cannot steer it. The host alone applies movement, world-clock advancement, due queues, healing, encounter rolls, and map-entry rules; the guest must never run those rules locally. This separates the input controller from game-state authority.
