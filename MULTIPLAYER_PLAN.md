...
Status: Phases 0 and 1 are complete on the `multiplayer-plan` branch. The Phase 2 transport, lobby, event journal, snapshot recovery, authenticated reconnect, complete gameplay-content manifest, and explicit first-contact fingerprint verification are implemented. Authority convergence is complete: live host and guest exploration inputs share the host command processor, replicas do not rerun rule-bearing pickup, skill, attack, script, or timed-queue work, and periodic corrections cover actors, both progressing character builds, critters, doors, items, world time, game globals, indexed map/script variables, and timed queues. Installed-data two-process scenarios now cover guest pathfinding and movement through completion, asynchronous scripted-door use and its final state, the deferred pickup callback and inventory mutation, loot initiation, targeted Traps use, and host-authoritative party XP. Each applies a full authoritative checkpoint and compares every section digest; the engine probe also proves the door script and random combat attack run only in the host process. Phase 2.5 deterministic exploration coverage is complete. Combat wire primitives exist, but live multiplayer attacks are intentionally blocked until authoritative phase and turn ownership are implemented.
...
- [x] Add host-authoritative targeted First Aid, Doctor, Lockpick, Steal, Traps, Science, and Repair commands for registered actors, critters, ground items, and doors. Human and `game_skill` agent input share ownership/phase/target validation and the acting-player build context; replicas apply only a presentation boundary, then receive script, roll, XP, object, and variable results through authoritative state. The installed-data Traps scenario proves command/result/event transport, full checkpoint convergence, and replay without guest-side rule execution. Registering and snapshotting non-door scenery skill targets remains part of the broader map-object result work below.

Remaining Phase 3A work is to register non-door scenery/containers that skills and scripts can mutate, replicate their concrete object state, and then drive a small non-combat quest end to end. The existing door/item/critter skill slice deliberately fails closed for an unregistered scenery target instead of falling back to local execution.
