...
`generateReconnectToken` seeds a CTR-DRBG from Mbed TLS platform entropy and fills each credential from the operating-system cryptographic random source. Tokens cross only the TLS transport. The first direct-IP connection records the host certificate through trust on first use; reconnects require that certificate pin before the token can leave the client.
