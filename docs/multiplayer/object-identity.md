...
The live world canonically registers all initial scenery, ground items, non-player critters, and their recursive inventories. Doors remain a separate snapshot section; other scenery carries its prototype, placement, art/frame, shared flags, light, and concrete union data. Registration checks the object type before applying the legacy portal predicate; that predicate reads scenery union fields and is not a safe classifier for actors or other object kinds. Recovery snapshots apply registered item ownership, position, mutable prototype and presentation state, and can recreate a missing described item. Timed queue entries also use these identities for rule-bearing owners; snapshot capture fails closed if such an owner is outside the registry. Script timed events are the exception because their handler resolves the script by SID and ignores the legacy owner pointer, so that pointer is normalized away. The stack-merge hook retires or rebinds the representative destroyed by `item_add_force`. The split hook keeps the moved object's ID and binds the copied remainder to a new host-assigned ID. Unregistered player items receive identity when they first enter an authoritative loot transfer or ground drop.

The engine object-erase boundary unregisters a replicated item before its storage is freed, including destruction initiated by a script opcode. An authoritative snapshot also removes registered replica items absent from the host, while avoiding a second deletion of nested contents. General script-created object identity remains outside this layer unless the object enters an existing authoritative inventory or snapshot path.
