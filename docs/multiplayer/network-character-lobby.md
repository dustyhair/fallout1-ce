...
The host retains a bounded journal of published exploration events for recovery. A guest detecting an event gap requests recovery from its last contiguous sequence. The host replays a retained suffix or sends a checked actor-and-door snapshot when the gap predates the journal. Pickup and loot events exist in the wire format but are not connected to live network input yet. Reconnect after the TCP connection closes is not yet enabled.
