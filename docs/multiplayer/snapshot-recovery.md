...
Snapshot version 2 contains the state needed to recover the live two-player experiment:
...
- Registered item identity, direct inventory holder or ground tile, elevation, and stack quantity.
...
`SnapshotReplica::apply` validates the entire snapshot before replacing replica state. A failed validation leaves the existing state unchanged. A successful application stores actors, doors, and items in canonical entity order. The engine adapter can move a tracked item between its authoritative holder and map position, including repairing a pickup whose journal event has expired.
...
The item section tracks objects already registered for live pickup and looting; it is not a general serialization of arbitrary script-created objects. Scripts, map variables, combat, and dialogue remain outside the recovery snapshot.
