...
Save and load handlers still read and write the legacy single-player arrays for compatibility. An active developer session also writes both routed builds to `MULTI.DAT` and restores them after the legacy world load. Guest sneak scheduling remains separate because the old sneak queue has one process-wide result flag.
