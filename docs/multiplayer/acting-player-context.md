...
Save and load handlers still read and write the legacy single-player arrays. Phase 1 will add a multiplayer sidecar for the two validated character builds. Guest sneak scheduling also remains separate because the old sneak queue has one process-wide result flag.
