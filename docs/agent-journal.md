...
22 game_rest 30
23 game_rest cancel
24 game_attack 42 0 8
25 game_combat_item 88
26 game_combat_item 88 42
27 game_reload 90 right
28 game_end_turn
29 game_talk 82
30 game_vote 42 2
31 game_trade_begin 2
...
`game_talk` requests host-authoritative dialogue with a registered nearby NPC. The `world_state.dialogue` object contains the current revision, reply, one-based options, and each player's visible vote; `game_vote <revision> <option>` submits a revision-keyed choice. `world_state.shared_activity` contains ordered, attributed party quest and discovery updates, separate from private character information.

`game_trade_begin` takes the other player's actor entity ID. `game_trade_offer`
takes the trade ID, displayed revision, cap amount, and an optional item ID and
